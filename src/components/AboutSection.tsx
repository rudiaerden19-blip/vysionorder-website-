...
import { platformRegisterUrl, SITE_NAME } from '@/lib/site'
...
const rightIcons = [
  (
    <>
      <path key="qr1" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 5h4v4H4V5zM16 5h4v4h-4V5zM4 15h4v4H4v-4z" />
      <path key="qr2" strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 15h2v2h-2v-2zm4 0h0m-2 2h2m2-2v2m0-2h2m-2 2v2" />
    </>
  ),
  (
    <path
...
const leftKeys = ['pickupDelivery', 'groupOrders'] as const
const rightKeys = ['qrOrder', 'loyalty'] as const
...
export default function AboutSection() {
  ...
  const leftFeatures: FeatureItem[] = leftKeys.map((key, i) => ({
    title: t(`about.features.${key}.title`),
    body: t(`about.features.${key}.body`),
    icon: leftIcons[i],
  }))

  const rightFeatures: FeatureItem[] = rightKeys.map((key, i) => ({
    title: t(`about.features.${key}.title`),
    body: t(`about.features.${key}.body`),
    icon: rightIcons[i],
  }))
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <header className="mx-auto max-w-3xl text-center">
          <p className="text-base font-medium text-brand sm:text-lg">{t('about.eyebrow')}</p>
          ...
          <p className="mt-5 text-lg leading-relaxed text-gray-600">
            {t('about.intro', { siteName: SITE_NAME })}
          </p>
        </header>
